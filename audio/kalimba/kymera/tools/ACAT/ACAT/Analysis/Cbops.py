...
class Cbops(Analysis.Analysis):
    """
    @brief An analysis plug in for ACAT. Extends the Analysis class to provide
    a capability for analysing a cbops chain/graph.
    """
    ...
    def __init__(self, **kwarg):
        ...
        try:
            self.buffer_types = self.debuginfo.get_enum(
                "cbops_buffer_type",
                None
            )
    ...
    def analyse_cbops_graph(
        ...
        """
        @brief The function analyses the cbops griper. It takes as a parameter the address of the
        cbops graph structure and extra optional parameters
        @param[in] self Pointer to the current object
        @param[in] address of the cbops graph structure
        @param[in] analyses_ops, if True it also analyses individual cbops operators in the graph
        @param[in] show_graph_buffers, if True the cbops buffers in the graph will also be processed
                   and displayed.
        """
        # get the cbops graph
    ...
    def _get_cbops_graph_buffers(self, cbops_graph, show_graph_buffers=False):
        ...
        # there are a total of num_io buffers for the graph, half of these are inputs
        # and half are output buffer(in current design at least). Not all of the cbops
        # buffers are valid(used), this function only extracts the indexes that are in use
        # by the cbops graph.
    ...
    def _get_cbops_buffer_type(self, buffer_type):
        """
        @brief The function to decode the cbops buffer type to a friendly string
        @param[in] self Pointer to the current object
    ...
    def _analyse_cbops_graph_operators(self, cbops_graph):
        ...
        try:
            ...
            for cbops_op in self.parse_linked_list(
            ...
            self.formatter.output("Override operator exists: {}".format(override_op_exist))

        except Exception as exception:
            # This should be error happening during parsing the linked list perhaps when
            # reading attempt from un-aligned or unmapped memory address
            # just output the type and message and end the analysis
            self.formatter.error(
    ...
    def _search_cbops_op_name_by_addr(self, cbops_op_p):
        """
        @brief The function finds operator structure name using address of cbops op
        @param[in] self Pointer to the current object
