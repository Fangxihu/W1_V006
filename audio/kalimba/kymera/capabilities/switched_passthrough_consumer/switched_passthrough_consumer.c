...
static void spc_process_data(OPERATOR_DATA *op_data, TOUCHED_TERMINALS *touched)
{
    ...
    {
        unsigned input_data, complete_data;
        tCbuffer *ip_buffer;
                ...
                input_num == get_input_number(opx_data->current_mode))
        {
            ...
            unsigned to_copy = MIN(complete_data, output_space);
            if (to_copy != 0)
            {
                unsigned copied = cbuffer->copy(op_buffer, ip_buffer, to_copy);
                if (copied != 0)
                {
                    L3_DBG_MSG3("SPC passed through [input%d] %d of %d",input_num,copied,to_copy);
        ...
        else if (complete_data != 0)
        {
            metadata_tag *mtag;
            unsigned b4idx, afteridx;

            L3_DBG_MSG2("SPC discard [input%d] %d of %d",input_num,complete_data);
