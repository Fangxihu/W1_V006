...
ruleEntry appConnRules[] =
{
    ...
    /*! \{
        Rules that are run on earbud connectable/non-connectable state changes */
    RULE(RULE_EVENT_CONNECTABLE_CHANGE,         rulePageScanUpdate,                 CONN_RULES_PAGE_SCAN_UPDATE),
    /*! /} */
...
#ifdef RECONNECT_HANDSET
		if(appPeerSyncIsPeerHandsetReconnecting())
		{
            RULE_LOG("ignore -peer is reconnecting");
			return RULE_ACTION_IGNORE;
		}
		else
#endif
...
static ruleAction ruleLinkLossConnectHandset(void)
{
    ...
    return ruleConnectHandset(RULE_CONNECT_LINK_LOSS);
}
...
{
    RULE_LOG("ruleUserConnectPeerHandset");
...
#ifdef RECONNECT_HANDSET
    appGetConnRules()->reason = RULE_CONNECT_NULL;
#endif
...
void ruleConnectSetReason(ruleConnectReason reason)
{
#if 0
    ruleConnectReason previous_reason = appGetConnRules()->reason;

    DEBUG_LOGF("ruleConnectSetReason, reason 0x%02x to 0x%02x", previous_reason, reason);

    /* Handle state exit functions */
    switch (previous_reason)
    {
        case RULE_CONNECT_USER:
            break;

        case RULE_CONNECT_PAIRING:
            break;

        case RULE_CONNECT_PEER_SYNC:
            break;
			
		case RULE_CONNECT_OUT_OF_CASE:
			break;
		
		case RULE_CONNECT_LINK_LOSS:
			break;
		
		case RULE_CONNECT_PEER_OUT_OF_CASE:
			break;
			
        default:
            DEBUG_LOGF("Attempted to exit unsupported state 0x%x", appGetConnRules()->reason);
            Panic();
            break;
    }
#endif
    /* Set new state */
    appGetConnRules()->reason = reason;
    ...
    switch (reason)
    {
        ...
        default:
            DEBUG_LOGF("Attempted to enter unsupported reason %d", reason);
            Panic();
            break;
    }
}

ruleConnectReason ruleConnectGetReason(void)
{
    return appGetConnRules()->reason;
}
