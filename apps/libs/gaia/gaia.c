...
static bool set_data_endpoint_mode(gaia_transport *transport, uint8 mode)
{
    switch(mode)
    {
        case GAIA_DATA_ENDPOINT_MODE_NONE:
            ...
            send_ack(transport, GAIA_VENDOR_CSR, GAIA_COMMAND_SET_DATA_ENDPOINT_MODE, GAIA_STATUS_SUCCESS, 0, NULL);
            return TRUE;
        ...
        default:
            send_ack(transport, GAIA_VENDOR_CSR, GAIA_COMMAND_SET_DATA_ENDPOINT_MODE, GAIA_STATUS_INVALID_PARAMETER, 0, NULL);
            return FALSE;
    }
}
...
static bool get_data_endpoint_mode(gaia_transport *transport)
{
    ...
    send_ack(transport, GAIA_VENDOR_CSR, GAIA_COMMAND_GET_DATA_ENDPOINT_MODE, GAIA_STATUS_SUCCESS, sizeof(mode), &mode);    
...
*/
static bool isHostUpgradeDataBytesReq(uint8* data, uint16 size_data)
{
...
DESCRIPTION
    Checks if the received data is UPGRADE_HOST_DATA_REQ

*/
static bool isHostUpgradeData(uint8* data)
{
    return (data != NULL && data[0] == GAIA_UPGRADE_HOST_DATA_REQ);
}
    ...
    if (gaia->upgrade_transport == NULL)
    {
        ...
        if (isRwcpEnabled())
        {
            ...
            if(transport->state.gatt.src)
            {
                ...
                VmalMessageSinkTask(StreamSinkFromSource(transport->state.gatt.src), &gaia->task_data);
...
void GaiaRwcpSendNotification(uint8 *payload, uint16 payload_length)
{
    ...
    /* Send notifications via ATT streams */
 ...
 */
void handle_upgrade_transport_data_cfm(uint8* data, uint16 size_data, upgrade_status_t status)
{
    ...
    if (isRwcpEnabled() && isHostUpgradeData(data))
        return;
...
*/
    ...
    case UPGRADE_TRANSPORT_DATA_CFM:
        if (gaia->upgrade_transport)
        {
            ...
            handle_upgrade_transport_data_cfm(cfm->data, cfm->size_data, status);
        }
        break;
...
*/
void gaiaProcessData(gaia_transport *transport, uint16 size_data, uint8 *data)
{
    RwcpServerHandleMessage(data, size_data);
    UNUSED(transport);
}
...
void GaiaOnTransportConnect(GAIA_TRANSPORT *transport)
{
    ...
    gaiaVoiceAssistantCheckHostVersion((gaia_transport *) transport);
}
