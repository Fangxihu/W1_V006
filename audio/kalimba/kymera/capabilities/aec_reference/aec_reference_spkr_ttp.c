...
static bool aec_reference_spkr_ttp_setup_timed_playback(AEC_REFERENCE_OP_DATA *op_extra_data)
{
    ...
    /* speaker cbops graph needs modification */
    if(NULL != op_extra_data->spkr_rate_monitor_op && op_extra_data->spkr_mic_in_same_clock)
    {
        /* rate monitor isn't required in timed playback mode,
         *
         * TODO_AEC_REFERENCE_TTP: this is might not be 100% safe, info is fetched
         * from the op is used in background though the chance of
         * conflict in near 0. Move this removing to background or
         * don't remove it at all (live with wasted MIPS).
         */
        cbops_remove_operator_from_graph(op_extra_data->spkr_graph, op_extra_data->spkr_rate_monitor_op);
...
static void aec_reference_spkr_ttp_adjust_rate(AEC_REFERENCE_OP_DATA *op_extra_data, int warp)
{
    ...
    else if(op_extra_data->spkr_ttp_adjust_type == RATEMATCHING_SUPPORT_SW)
    {
        ...
        cbops_sra_set_rate_adjust(op_extra_data->spkr_sw_rateadj_op,
                                  op_extra_data->num_spkr_channels,
                                  warp);
    }
}
