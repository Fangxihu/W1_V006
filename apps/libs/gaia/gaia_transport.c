...
void gaiaTransportProcessSource(gaia_transport *transport)
{
    ...
    if (SourceIsValid(source))
    {
        ...
        locals->data_length = SourceSize(source);
        locals->data = (uint8 *) SourceMap(source);
        locals->packet = NULL;
        locals->flags = 0;
        locals->check = 0;
        ...
        if (locals->data_length >= GAIA_OFFS_PAYLOAD) {
            ...
            if (locals->packet_length == locals->expected) {
                ...
                if (!gaia->upgrade_large_data.in_progress) {
                    SourceDrop(source, locals->data_length);
...
bool gaiaTransportHandleMessage(Task task, MessageId id, Message message) {
    ...
    GAIA_TRANS_DEBUG(("gaiaTransportHandleMessage==> gaiaTransportGattHandleMessage\n"));
    if (gaiaTransportGattHandleMessage(task, id, message))
    return TRUE;
#endif
    return FALSE;
}
