...
static void configureOperatorsInChain(kymera_chain_handle_t chain, audio_codec_t decoder,
...
void audioInputA2dpConnectHandler(audio_input_context_t *ctx, Task task, const AUDIO_PLUGIN_CONNECT_MSG_T *msg)
{
    ...
    if(AudioPluginA2dpTaskIsBroadcaster(task))
    {
        ...
        configureOperatorsInChain(ctx->chain, codec, ctx->sample_rate, a2dpParams);
