...
uint32 PioDebounce32Bank(uint16 bank, uint32 mask, uint16 count, uint16 period)
{
    ...
    L4_DBG_MSG4("PIO trap: PioDebounce32Bank(%d, 0x%x, %d, %d)", bank, mask, count, period);
    ...
    piodebounce_config(bank, mask, count, period, pio_handler);
    return 0;
}
...
void pio_handler(uint16 bank)
{
    L4_DBG_MSG1("PIO trap: message to task, bank %d", bank);
    trap_pio_send_msg(pio_task, bank, piodebounce_get(bank), get_milli_time());
}
