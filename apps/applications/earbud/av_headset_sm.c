...
#ifdef RECONNECT_HANDSET
	sm->rec_state = RECONNECT_STATE_NULL;
#endif
...
bool appSmRuleConnectUser(void)
{
	DEBUG_LOG("appSmRuleConnectUser-get!");
	smTaskData *sm = appGetSm();
	return sm->rule_connect_user;
}

void appSmSetRuleConnectUser(bool value)
{
	DEBUG_LOG("appSmRuleConnectUser-set!");
	smTaskData *sm = appGetSm();
	sm->rule_connect_user = value;
}

#if 1
bool appSmReconnectAndPairing(void)
{
	DEBUG_LOG("appSmReconnectAndPairing-get!");
    smTaskData *sm = appGetSm();
    return sm->reconnect_and_pairing;
}

void appSmSetReconnectAndPairing(void)
{
	DEBUG_LOG("appSmSetReconnectAndPairing-set!");
    smTaskData *sm = appGetSm();
    sm->reconnect_and_pairing = TRUE;
    //MessageSend(appGetSmTask(), SM_INTERNAL_SEND_PEER_SYNC, NULL);
}

void appSmClearReconnectAndPairing(void)
{
	DEBUG_LOG("appSmClearReconnectAndPairing-clear!");
    smTaskData *sm = appGetSm();
    sm->reconnect_and_pairing = FALSE;
    //MessageSend(appGetSmTask(), SM_INTERNAL_SEND_PEER_SYNC, NULL);
}
#endif
...
/* This function is called to change the applications state, it automatically
   calls the entry and exit functions for the new and old states.
*/
void ReconnectSetState(reconnectState new_state)
{
    reconnectState previous_state = appGetSm()->rec_state;

    DEBUG_LOGF("ReconnectSetState, state 0x%02x to 0x%02x", previous_state, new_state);

    /* Handle state exit functions */
    switch (previous_state)
    {
        case RECONNECT_STATE_NULL:
            break;

        case RECONNECT_STATE_ING:
            break;

        case RECONNECT_STATE_END:
            break;

        default:
            DEBUG_LOGF("Attempted to exit unsupported state 0x%x", appGetSm()->rec_state);
            Panic();
            break;
    }

    /* Set new state */
    appGetSm()->rec_state = new_state;
