...
if __name__ == "__main__":
    import optparse
    usage = """This script parse a config file and creates(/updates if necessary) the builddefs.mkf
and <CONFIG>.mkf for the build. It checks if any of the %cpp defines changed from
the previous build and if so it marks the relevant files for rebuild. """

    def cback(option, opt_str, value, parser):
        """Function used to create the list of download capabilities after the -d option"""
        args=[]
    ...
    parser.add_option("-o", action="store", type="string", dest="output_path", default="", help="The target build directory")
    parser.add_option("-k", action="store", type="string", dest="kymera_path", default="", help="Kymera root directory")
    parser.add_option("-f", action="store", type="string", dest="config_file", default="", help="The config file.")
    parser.add_option("--dnld-build-name", action="store", type="string", dest="build_name", default="", help="Build name of the ELF (download only)")
    parser.add_option("--dnld-config-files", action="callback", callback=cback, dest="download_cfg_files", help="List of config files used by capabilities (download only)")
    ...
    if options.download_cfg_files != None and options.build_name != "":
