...
class Fault(Analysis.Analysis):
    """
    @brief This class retrieves information about faults that have occurred
        on the chip. An array faultinfo records instances of faults.
        The last fault to have occurred has it's diatribe and id stored
        in the preserve memory block.
    """
    ...
    def run_all(self):
        ...
        Displays the contents of the faults from faultinfo array and the preserve block.
    ...
    def _last_faultrecord(faultlist):
        """ last_faultrecord(iterable) -> generates (thing, islast) pairs
            ...
            last item in the sequence. Used to determine last element in faultinfo[].
        """
    ...
    def analyse_preserve_block_fault(self):
        ...
        if last_fault_id != 0 and (last_fault_id & 0xFF0000 == 0):
            # for Crescendo, the preserved block is populated with random values,
            # so unless  a fault has occurred the fault_id will most probably
            # be invalid
            try:
                ...
                self.formatter.alert(
                    'Last recorded fault on chip: fault id %s and diatribe 0x%x' %
