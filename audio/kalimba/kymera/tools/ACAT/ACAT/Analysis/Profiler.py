...
class Profiler(Analysis.Analysis):
    """
    @brief This class encapsulates analyses for measuring CPU runtime
    performance.
    """
    ...
    def run_kymera_builtin(self, detailed_debug=False, raw_debug=False):
        ...
        if profiler_head_ptr != 0:
            ...
            for profiler_entry in self.parse_linked_list(
                ...
                meas_width = max(map(len, measurements))
                ...
                param_width = max(7, max(map(len, params)))
                raw = {}
                ...
                for meas in measurements:
                    ...
                    for typ in on_chip_param_suffixes:
                        ...
                        mem = profiler_entry.get_member(meas + "_" + typ)
                        if mem is not None:
                            cols[typ] = mem.value
                ...
                if len(rows) > 0:
                    ...
                    builtin_profiler_log(
                        "%*s  " % (meas_width, "") +
                        "  ".join(map(lambda s:"%*s" % (param_width, s),
                                      params)) + "\n"
                    )
                    # Table body
                    for meas in measurements:
                        if meas in rows:
                            cols = [((param_formats[i] % rows[meas][params[i]])
