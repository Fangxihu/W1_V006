...
/*! \brief Handle encryption confirmation

    This function is called to handle a CL_SM_ENCRYPT_CFM message, this message is sent from the
    connection library when an encrption is enabled

    \param  cfm     The confirmation to handle.
*/
void appAuthHandleClSmEncryptionCfm(const CL_SM_ENCRYPT_CFM_T* cfm)
{
    DEBUG_LOG("appAuthHandleClSmEncryptionCfm");
    ...
    if(cfm->status == success && cfm->encrypted)
    {
        ...
        if (SinkGetBdAddr(cfm->sink, &tpaddr))
            appAuthHandleEncryptionChange(&tpaddr.taddr.addr, cfm->encrypt_type);
    }
}
