...
static uint32 mixerGetOutputSampleRate(connection_type_t connection_type, uint32 input_sample_rate)
{
    ...
    {
        uint16 i2s_voice_resampling_frequency = CsrI2SVoiceResamplingFrequency();
...
*/
static void mixerHardwareConnect(connection_type_t connection_type, uint32 sample_rate)
{
    ...
    hw_connect_data.sample_rate= sample_rate;
    ...
    AudioHardwareConnect(&hw_connect_data);
}
...
static void mixerCreateAndConnect(const audio_mixer_connect_t* connect_data)
{
    ...
    if(!mixer_already_created)
    {
        ...
        mixerHardwareConnect(connect_data->connection_type, connect_data->sample_rate);
    }
    else
    {        
