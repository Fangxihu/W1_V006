...
class DebugLog(Analysis.Analysis):
    """
    @brief This class encapsulates an analysis for Debug Log decode.
    ...
    @is_debuglog_enabled_decorator
    def poll_debug_log(self, wait_for_keypress=True):
        """
        @brief Polls the debug_log buffer. If the contents of the buffer changes then
...
class Logging(threading.Thread):
    ...
    def __init__(self, chipdata, debuginfo, polling, stop_event, formatter):
        ...
        # When polling on a live chip we need to remember where we last read up
        # to; by starting at -10 even if we add 1 to do some checks it doesn't
        # go positive
        self.last_read = -10
        ...
        try:
            ...
            # to addressable units since get_data takes the size in addr units
            # !! Because debugBufferSizeMask is size-1.
    ...
    def _get_formatter_string(self, string_ptr, arguments):
        """
        @brief Searches for the formatter string in the debug information. Also checks
        the downloadable capabilities.

        @param[in] self Pointer to the current object
        @param[in] string_ptr Pointer to the formatter string
        @param[in] arguments Arguments for the formatter string. Note this can change.
        """
        ...
        if region == 'DBG_DWL':
            ...
            try:
                if arguments:
                    ...
                    if elf_id is not None:
                        ...
                        arguments.pop(0)
                # The linker will put the debug messages for downloadable capabilities
                # to 0x15500000, but the elf for some reason leaves it in 0x13500000.
                # Count for the difference 0x15500000 -  0x13500000 = 0x2000000#
                ...
                ) % (cu.hex(string_ptr), cu.hex(self.current_elf_id))
            ...
            if patch is None:
                ...
                try:
                    ...
                    # Count for the difference 0x14500000 -  0x13500000 = 0x1000000
            ...
            try:
                ...
                mystr = None
            if mystr is None:
                mystr = "@@@@ ERROR: Cannot find %s debug string." % cu.hex(
                    string_ptr
                )
        return mystr
    ...
    def decode_log(self):
        ...
        This function could be called from outside of this thread. A lock is used to
        makes sure it is only running on one thread.
        @param[in] self Pointer to the current object
        """
    ...
    def _decode_log(self):
        ...
        # * We can't assume that a value > $flash.debugdata.__Base is a string pointer
        #   (it could just be a large number)
        # * String pointers can in some circumstances be arguments to other strings
        #   (more details below).
        # * The buffer may not be completely full
        # * The buffer may have wrapped unevenly, so that the first few arguments after
        #   buffer_wrp are orphaned from the corresponding string.
        #
        ...
        while True:
            ...
            if region == 'DEBUG' or region == 'DBG_DWL'or region == 'DBG_PTCH':
                ...
                # Now it could just so happen that this value is an argument to printf
                # AND a valid debug string pointer, like:
                ...
                # of arguments as we have in a[], but that doesn't actually help
                # in this case since it doesn't allow us to distinguish between
                # a string that simply has no arguments, and a string that is an
                # argument to another string.
                # To avoid painful look-ahead, the simple way to solve this is
                # to just assume every complete-looking string is actually complete,
                # and put it into tmp_debug_log[]. If we then come across a string
                # that doesn't seem to have enough arguments, we backtrack.
                # Note that we only support one level of nesting here; if someone has
                # put a formatted string inside a formatted string then all
                # bets are off.
                ...
                if mystr.count('%') > len(arguments):
                    # We're missing some arguments.
                    # We probably put them into tmp_debug_log[], thinking they were
                    # complete strings.
                    ...
                    if num_args_missing == mystr.count('%s'):
                        ...
                        if Arch.addr_per_word != 4:
                            ...
                            # Napier the arguments in tmp_debug_log[]
                            # are in the right order, because the order in which
                            # the arguments are stored. For Blucore platforms,
                            # like Gordon and Rick, the array slice is in the
                            # wrong order.
                            temp_arguments.reverse()
                        arguments = temp_arguments + arguments
                        # now arguments[] contains what it would have held, had we not put
                        # the missing args into tmp_debug_log[]. Last thing to do is
                        # disavow them.
    ...
    def run(self):
        """
        @brief Polls the debug log buffer repeatedly. This is called in a separate
