 ...
 *
 *---------------------------------------------------------------------------*/
static void handleDataSegment(uint8 sequence_number, uint8 *data, uint16 size )
{
    /*
...
void RwcpServerHandleMessage(uint8 *data, uint16 size)
{
    uint8 rwcp_header;
        ...
        case RWCP_SYN_RCVD:
            ...
            switch(command)
            {
                ...
                case RWCP_CLIENT_CMD_DATA:
                    ...
                    g_server_data.out_of_sequence_status = FALSE;
                    handleDataSegment( sequence_number, data, size);
