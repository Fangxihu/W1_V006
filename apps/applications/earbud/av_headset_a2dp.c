                                                    ...
                                                    const AV_INTERNAL_A2DP_CONNECT_REQ_T *req)
{
    ...
    switch (appA2dpGetState(theInst))
    {
        case A2DP_STATE_DISCONNECTED:
        {
			...
			DEBUG_LOGF("req->num_retries = %d", req->num_retries);
...
/*! \brief Find the start of the service category in CODEC capabilities.
    ...
    \param caps_ptr [IN,OUT] Must be set to point to a service category definition
    ...
    \param size_caps_ptr [OUT] Set to the total size of the category, or 0 if not
    found/malformed.
*/
