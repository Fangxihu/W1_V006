...
static void ProcessOTAControlMessage(uint8 ota_msg_id, const uint8* payload, int payload_size)
{
    ...
    switch (ota_msg_id)
    {
        ...
        case SFWD_OTA_MSG_RECONNECT_HANDSET:
            ...
            //theScoFwd->earbud_reconnect_handset = TRUE;
			if((ReconnectGetState() != RECONNECT_STATE_END) && (!appSmStateInCase(appGetState())))
			{
	            appSmHandleReconnectHandset();
			}
			else
			{
				DEBUG_LOG("ignore!");
		...
		case SFWD_OTA_MSG_RULE_CONNECT_USER:
			DEBUG_LOG("SFWD_OTA_MSG_RULE_CONNECT_USER");
			if(!appSmStateInCase(appGetState()))
			{
				appSmHandleReconnectHandset();
			}
			else
...
static void appScoFwdExitConnectedActiveSend(void)
{
    ...
    /* clean up local sfwd master forwarding state */
...
//    appScoFwdSetState(SFWD_STATE_ACTIVE_SEND);
...
{
    DEBUG_LOGF("appScoFwdHandleEnableForwarding SFWD state %u", appScoFwdGetState());
...
static void appScoFwdHandleDisableForwarding(void)
{
    ...
    DEBUG_LOGF("appScoFwdHandleDisableForwarding SFWD state %u", appScoFwdGetState());
...
static void appScoFwdHandlePhyStateChangedInd(const PHY_STATE_CHANGED_IND_T *ind)
{
    ...
    switch (ind->new_state)
    {
        ...
        {
            if ((theScoFwd->state == SFWD_STATE_CONNECTED_ACTIVE_SEND) ||
                (theScoFwd->state == SFWD_STATE_CONNECTED_ACTIVE))
            {
...
void appScoFwdReconnectHandset(void)
{
	#if 0
    scoFwdTaskData *theScoFwd = appGetScoFwd();
	if(!theScoFwd->earbud_reconnect_handset)	{
	    DEBUG_LOG("appScoFwdReconnectHandset");
	    SendOTAControlMessage(SFWD_OTA_MSG_RECONNECT_HANDSET);
	    //theScoFwd->earbud_reconnect_handset = TRUE;
	}
	#endif
	DEBUG_LOG("appScoFwdReconnectHandset-send!");
	if(!appPeerSyncIsPeerInCase())
	{
		SendOTAControlMessage(SFWD_OTA_MSG_RECONNECT_HANDSET);
	}
}
void appScoFwdRuleConnectUser(void)
{
	DEBUG_LOG("appScoFwdRuleConnectUser-send!");
	if(!appPeerSyncIsPeerInCase())
	{
