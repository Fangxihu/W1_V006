...
class Operator(object):
    ...
    def __init__(self, op_entry, helper):
        ...
        # Additional informations for an operator
        ...
        # Continue with the operator command handlers (it is last because many of
        # these function are just a reuse from the basic operator).
        if self.extra_op_data_type is None:
    ...
    def _channel_string(self):
        ...
        if channel_p != 0:
            ...
            if first_channel != 0:
                channels_str = ""
                for chan in self.helper.parse_linked_list(
                        first_channel, "next_active"
                    ):
                    chanstr = str(chan)
    ...
    def _common_base_class_data(self):
        ...
        if "ENCODE" in self.cap_data.name or \
            "encode" in self.cap_data.name:
            return self._encoder_string()
        return self._channel_string()

    def _desc_str(self):
        mystr = ('Connection ID: ' + cu.hex(self.con_id) + '\n')
        mystr += ('Task ID: ' + cu.hex(self.task_id) + '\n')
        mystr += ('Creator client ID: ' +
                  cu.hex(self.creator_client_id) + '\n')
        mystr += (
            'State: ' + self.helper.debuginfo.get_enum(
                'OP_STATE', self.state
            )[0].replace("OP_", "") + '\n'
        )
        ...
        for port in range(self.cap_data.max_sources):
            ...
            if associated_ep is not None:
                # associated_ep could be None if we've destroyed (or never created)
                # the Endpoint to go with this operator.
        ...
        if self.extra_op_data_type is None:
            mystr += (
                "extra_op_data address: " + cu.hex(self.extra_op_data_address)
            )
            elf_id = self.cap_data.elf_id
            if self.helper.debuginfo.is_elf_loaded(elf_id):
                ...
                mystr += (
                    "\nopmgr.set_extra_op_data_type( " + cu.hex(self.op_ep_id) +
                    ",\n\t\"<the type used to cast op_data->extra_op_data to>\")"
                )
            else:
                mystr += "\nCannot display operator.\n"\
                    "Bundle with elf id {0} is missing!\n"\
                    "Use -j option or load_bundle(r\"<path>\") in interactive "\
                    "mode to add bundles to ACAT!".format(cu.hex(elf_id))
...
def source_sync_str(self):
    ...
    try:
        ...
        for sink_group in self.helper.parse_linked_list(
                sink_groups, "common.next", sink_groups.type
            ):
        ...
        for source_group in self.helper.parse_linked_list(
                source_groups, "common.next", source_groups.type
            ):
...
def rtp_str(self):
    ...
    try:
        ...
        clone_frame_buffer = self.extra_op_data.get_member("clone_frame_buffer").value
        op_str += "\n clone_frame_buffer:\n"
        ...
        internal_buffer = self.extra_op_data.get_member("internal_buffer").value
        op_str += "\n internal_buffer:\n"
    ...
    try:
        ...
        for channel in self.helper.parse_linked_list(channels, "next"):
            ...
            if internal_buff != 0:
                ...
                cbuffer_var = self.helper.chipdata.cast(internal_buff, "tCbuffer")
        ...
        if data < 0:
            data += metadata_size
        percent = (float(data) * 100) / metadata_size
        op_str += "Channel 1 : usage %3.2f%% data %d size %d" % (percent, data, metadata_size)

        return op_str

    except (ct.DebugInfoNoVariable, AttributeError):
        return Operator._desc_str(self)
...
class Capability(object):
    ...
    def __init__(self, cap_entry, helper, cap_elf_id):
        ...
        try:
            ...
            # Not a built in capability. Capability IDs are allocated as follows:
            ...
            if self.id >= 0x4000 and self.id <= 0x7FFF:
                self.name = "(Qualcomm_Provided_Capability:" + cu.hex(self.id) + ")"
            elif self.id >= 0x8000 and self.id <= 0xBFFF:
                # Probably these IDs will never be used.
                self.name = "(Reserved_Capability:" + cu.hex(self.id) + ")"
            elif self.id >= 0xC000 and self.id <= 0xC0FF:
                self.name = "(Customer_Capability:" + cu.hex(self.id) + ")"
            elif self.id >= 0xC100 and self.id <= 0xFFFF:
                self.name = "(Qualcomm_eXtension_Capability:" + cu.hex(self.id) + ")"
            else:
                # could be any downloaded capability
                self.name = "(Downloaded_Capability:" + cu.hex(self.id) + ")"
    ...
    def title_str(self):
        ...
        @param[in] self Pointer to the current object
        """
        return self.name + ' id: ' + cu.hex(self.id)
    ...
    def desc_str(self):
        ...
        @param[in] self Pointer to the current object
        """
        mystr = ('version: ' + cu.hex(self.version) + '\n')
...
def operator_factory(operator_var, analysis):
    ...
    if operator.cap_data.name == "SPLITTER" or operator.cap_data.name == "DOWNLOAD_SPLITTER":
        # use special descriptor function for source sync capability
        operator._desc_str = types.MethodType(splitter_str, operator)

    if operator.cap_data.name == "RTP_DECODE" or operator.cap_data.name == "DOWNLOAD_RTP_DECODE":
        # use special descriptor function for source sync capability
...
class Opmgr(Analysis.Analysis):
    """
    @brief This class encapsulates analysis for operators
    @param[in] self Pointer to the current object
    """
    ...
    def __init__(self, **kwarg):
        ...
        try:
            ...
            raise ct.AnalysisError("""OPMGR analysis doesn't work without Stream analysis.
Please check the available analyses and make sure that the Stream is ahead of OPMGR.""")
    ...
    def ep_id_from_port_opid_and_dir(port, op_ep_id, direction):
        """
        @brief Takes a port number (e.g. 0), op_ep_id (e.g. 0x4040) and direction
    ...
    def get_oplist(self, mode='id'):
        ...
        if mode == 'entry':
            ...
            return [operator_factory(operator, self) for operator in op_entries]
        elif mode == 'name':
            return_list = []
    ...
    @Analysis.cache_decorator
    def get_extra_op_data_type(
            self, function_address, elf_id
        ):
        """
        @brief It takes the function address and searches for the nearest label
        ...
        if nearest_label is not None:
            func_address = cu.hex(nearest_label.address)
        else:
            func_address = cu.hex(function_address)
