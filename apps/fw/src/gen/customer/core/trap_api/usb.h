 ...
 *    
 *    This API is used to register both the USB interface descriptor details, and
 *  any class specific
 ...
 *  \return An interface number on success, else returns usb_interface_error. Possible
 *  reasons for usb_interface_error include insufficient pmalloc space for new
 ...
 *  \return TRUE if the descriptor was successfully added, else FALSE. Descriptor type for
 *  a class specific endpoint descriptor will contain the address of the endpoint
 *  in the upper 8 bits. The upper 8 bits should be 0 otherwise. This API is
 *  normally used to add HID report or class specific endpoint descriptors.
 * 
 * \note This trap may NOT be called from a high-priority task handler
 * 
 * \ingroup trapset_usb
 ...
 *  \return TRUE if the Interface Association descriptor was added correctly, else FALSE. A
 *  FALSE value will be returned if an invalid if_num is given.
 * 
 * \note This trap may NOT be called from a high-priority task handler
 * 
 * \ingroup trapset_usb
 ...
 *  \return On success, returns a value corresponding to the bConfigurationValue of the new
 *  configuration. This will start at 2 for the first extra configuration created.
 *  On error, returns zero. Possible reasons for an error include calling the
 *  function after interfaces have been added, insufficient pmalloc space for new
 *  firmware data structures, BlueCore is already enumerated on the bus,
 *  PSKEY_HOST_INTERFACE is not set to USB or PSKEY_USB_VM_CONTROL is FALSE.
 * 
 * \note This trap may NOT be called from a high-priority task handler
