...
static bool usbEnumerateHidConsumerTransportControl(void)
{
    ...
    (void) VmalMessageSinkTask(StreamUsbClassSink(intf), &HID_CONS_TASK);
...
static bool usbEnumerateHidKeyboard(bool consumer_active)
{
    ...
    (void) VmalMessageSinkTask(StreamUsbClassSink(intf), &HID_KEYB_TASK);
...
static bool usbEnumerateHidDatalinkControl(void)
{
    ...
    (void) VmalMessageSinkTask(device->datalink_class_sink, &HID_DATA_TASK);
