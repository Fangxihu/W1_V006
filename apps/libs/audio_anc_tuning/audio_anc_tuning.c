...
static void connectMic(audio_mic_params * mic_params, audio_channel channel, tuning_operator_input_terminals_t terminal)
{
    Operator tuning_operator = ChainGetOperatorByRole(tuning_chain, tuning_role);
    Source mic_source = AudioPluginMicSetup(channel, *mic_params, ANC_TUNING_MODE_SAMPLE_RATE);
...
static audio_channel getAudioChannel(anc_path_enable mic_path)
{
    ...
    switch(mic_path)
    {
        ...
        case feed_forward_right:
            if((anc_mic_params.enabled_mics & feed_forward_left)
                    && (anc_mic_params.feed_forward_left.instance == anc_mic_params.feed_forward_right.instance))
            {
        ...
        case feed_back_left:
            if((anc_mic_params.enabled_mics & feed_forward_left)
                    && anc_mic_params.feed_forward_left.instance == anc_mic_params.feed_back_left.instance)
            {
                mic_channel = AUDIO_CHANNEL_B;
            }
            break;
        case feed_back_right:
            if(((anc_mic_params.enabled_mics & feed_back_left)
                    && anc_mic_params.feed_back_left.instance == anc_mic_params.feed_back_right.instance)
                || ((anc_mic_params.enabled_mics & feed_forward_right)
                    && anc_mic_params.feed_forward_right.instance == anc_mic_params.feed_back_right.instance))
            {
                mic_channel = AUDIO_CHANNEL_B;
            }
            break;
...
static void connectAncMicrophones(void)
{
    switch(anc_mic_params.enabled_mics)
    {
        ...
        case hybrid_mode_right_only:
            ...
            {
                connectMic(&anc_mic_params.feed_back_left, getAudioChannel(feed_back_left), mic_1_left_terminal);
                connectMic(&anc_mic_params.feed_forward_left, getAudioChannel(feed_forward_left), mic_2_left_terminal);
            }
            if (isAncConfigRightMicEnabled())
            {
                connectMic(&anc_mic_params.feed_back_right, getAudioChannel(feed_back_right), mic_1_right_terminal);
                connectMic(&anc_mic_params.feed_forward_right, getAudioChannel(feed_forward_right), mic_2_right_terminal);
            }
            break;
        ...
        case feed_forward_mode_right_only:
            if (isAncConfigLeftMicEnabled())
                connectMic(&anc_mic_params.feed_forward_left, getAudioChannel(feed_forward_left), mic_1_left_terminal);
            if (isAncConfigRightMicEnabled())
                connectMic(&anc_mic_params.feed_forward_right, getAudioChannel(feed_forward_right), mic_1_right_terminal);
            break;
...
{
    audio_mic_params monitor_mic = getMonitorMic();

    if (isAncConfigLeftMicEnabled())
    {
        connectMic(&monitor_mic, AUDIO_CHANNEL_A, monitor_left_terminal);
        setMonitorMicGain(&monitor_mic, AUDIO_CHANNEL_A);
    }

    if (isAncConfigRightMicEnabled())
    {
        connectMic(&monitor_mic, AUDIO_CHANNEL_B, monitor_right_terminal);
        setMonitorMicGain(&monitor_mic, AUDIO_CHANNEL_B);
    }
}
...
static Source getMonitorMicToSynchroniseWith(void)
{
    ...
    if(isAncConfigLeftMicEnabled())
    {
        sync_source = AudioPluginGetMicSource(getMonitorMic(), AUDIO_CHANNEL_A);
    }
    else
    {
        sync_source = AudioPluginGetMicSource(getMonitorMic(), AUDIO_CHANNEL_B);
    }
...
static void synchroniseMicrophones(void)
{
    ...
    {
        SourceSynchronise(sync_source, AudioPluginGetMicSource(getMonitorMic(), AUDIO_CHANNEL_B));
    }

    if(anc_mic_params.enabled_mics && feed_forward_left)
    {
        SourceSynchronise(sync_source, AudioPluginGetMicSource(anc_mic_params.feed_forward_left, getAudioChannel(feed_forward_left)));
    }

    if(anc_mic_params.enabled_mics && feed_forward_right)
    {
        SourceSynchronise(sync_source, AudioPluginGetMicSource(anc_mic_params.feed_forward_right, getAudioChannel(feed_forward_right)));
    }

    if(anc_mic_params.enabled_mics && feed_back_left)
    {
        SourceSynchronise(sync_source, AudioPluginGetMicSource(anc_mic_params.feed_back_left, getAudioChannel(feed_back_left)));
    }

    if(anc_mic_params.enabled_mics && feed_back_right)
    {
        SourceSynchronise(sync_source, AudioPluginGetMicSource(anc_mic_params.feed_back_right, getAudioChannel(feed_back_right)));
    }
}
...
{
    ChainSetDownloadableCapabilityBundleConfig(AudioAncTuningGetBundleConfig());
...
static void disconnectAncMicrophones(void)
{
    if(anc_mic_params.enabled_mics & feed_forward_left)
    {
        disconnectSource(AudioPluginGetMicSource(anc_mic_params.feed_forward_left, getAudioChannel(feed_forward_left)));
    }
    if(anc_mic_params.enabled_mics & feed_forward_right)
    {
        disconnectSource(AudioPluginGetMicSource(anc_mic_params.feed_forward_right, getAudioChannel(feed_forward_right)));
    }
    if(anc_mic_params.enabled_mics & feed_back_left)
    {
        disconnectSource(AudioPluginGetMicSource(anc_mic_params.feed_back_left, getAudioChannel(feed_back_left)));
    }
    if(anc_mic_params.enabled_mics & feed_back_right)
    {
        disconnectSource(AudioPluginGetMicSource(anc_mic_params.feed_back_right, getAudioChannel(feed_back_right)));
    }
}
...
static void disconnectMonitorMicrophones(void)
{
    ...
    if (isAncConfigLeftMicEnabled())
        disconnectSource(AudioPluginGetMicSource(monitor_mic, AUDIO_CHANNEL_A));

    if (isAncConfigRightMicEnabled())
        disconnectSource(AudioPluginGetMicSource(monitor_mic, AUDIO_CHANNEL_B));
}
