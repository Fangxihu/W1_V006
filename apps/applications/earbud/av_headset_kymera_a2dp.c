...
bool appKymeraHandleInternalA2dpStart(const KYMERA_INTERNAL_A2DP_START_T *msg)
{
    ...
    DEBUG_LOGF("appKymeraHandleInternalA2dpStart, state=%u", theKymera->state);
...
void appKymeraHandleInternalA2dpStop(const KYMERA_INTERNAL_A2DP_STOP_T *msg)
{
    ...
    DEBUG_LOGF("appKymeraHandleInternalA2dpStop, state(%u) seid(%u)", theKymera->state, seid);
    ...
    if (appA2dpIsSeidNonTwsSink(seid) || appA2dpIsSeidTwsSink(seid))
    {
        switch (theKymera->state)
        {
            ...
            default:
                ...
                DEBUG_LOGF("appKymeraHandleInternalA2dpStop, invalid state(%u)", theKymera->state);
            break;
        }
    }
    else if (appA2dpIsSeidSource(seid))
