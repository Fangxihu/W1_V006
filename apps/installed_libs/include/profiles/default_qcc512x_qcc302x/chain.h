...
and MAKE_CHAIN_CONFIG_WITH_STREAMS(id, ucid, operators, streams) macro is 
...
Macro MAKE_CHAIN_CONFIG_WITH_STREAMS(id, ucid, operators, streams) can be used 
...
Each path is described by an array of operator_stream_node_t structures.
...
\code
{
    ...
    stream_with_input,  // this path forms and input of the chain, but terminates internaly within the chain
...
/*! Describes a node as part of a data path (see operator_stream_t) 
 */
typedef struct
{
...
typedef struct
{
    ...
    /*! Pointer to an array of operator_stream_node_t structures. Each node
...
Returns a Sink which represents the chain input for a given stream role.
...
Returns a Source which represents the chain ouput for a given stream role.
