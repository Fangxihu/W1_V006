...
bool connectionBluestackHandlerDmCrypto(const DM_UPRIM_T *message)
{
    switch (message->type)
    {
        ...
        {
            CL_DEBUG(("DM_CRYPTO_GENERATE_PUBLIC_PRIVATE_KEY_CFM\n"));
        ...
        {
            CL_DEBUG(("DM_CRYPTO_GENERATE_SHARED_SECRET_KEY_CFM\n"));
        ...
        {
            CL_DEBUG(("DM_CRYPTO_ENCRYPT_CFM\n"));	
...
void connectionHandleGeneratePublicPrivateKeyCfm(DM_CRYPTO_GENERATE_PUBLIC_PRIVATE_KEY_CFM_T *cfm)
{
    ...
    if(theAppTask)
    {
        ...
        if (message->status == cl_crypto_success)
        {
            ...
            memmove(message->public_key, cfm->public_key, CL_CRYPTO_PUBLIC_KEY_SIZE);
            memmove(message->private_key, cfm->private_key, CL_CRYPTO_PRIVATE_KEY_SIZE);
...
void connectionHandleGenerateSharedSecretKey(DM_CRYPTO_GENERATE_SHARED_SECRET_KEY_CFM_T * cfm)
{
    ...
    if(theAppTask)
    {
        ...
        if (message->status == cl_crypto_success)
        {
            ...
            memmove(message->shared_secret_key, cfm->shared_secret_key, CL_CRYPTO_SECRET_KEY_SIZE);
            message->key_type = connectionConvertEccType(cfm->key_type);
        }
...
void connectionHandleAesEncrypt(DM_CRYPTO_ENCRYPT_CFM_T * cfm)
{
    ...
    if(theAppTask)
    {
        ...
        if (message->status == cl_crypto_success)
        {
            ...
            memmove(message->encrypted_data, cfm->encrypted_data, CL_CRYPTO_AES_DATA_SIZE);
        }
    ...
    if(theAppTask)
    {
        ...
        if (message->status == cl_crypto_success)
        {
            if((message->operation == cl_crypto_single_block) || (message->operation == cl_crypto_data_end))
            {
                ...
                memmove(message->hash, cfm->hash, CL_CRYPTO_SHA_HASH_SIZE);
            }
        }
