...
#define SHA_DATA_OCTET_SIZE ((sizeof(uint16) == 1) ? CL_CRYPTO_SHA_DATA_SIZE * 2: CL_CRYPTO_SHA_DATA_SIZE)
...
void ConnectionGenerateSharedSecretKey(Task theAppTask, cl_ecc_type key_type, uint16 private_key[CL_CRYPTO_LOCAL_PVT_KEY_SIZE], uint16 public_key[CL_CRYPTO_REMOTE_PUB_KEY_SIZE])
{
    ...
    if (public_key)
    {
        memmove(&prim->remote_public_key, public_key, CL_CRYPTO_REMOTE_PUB_KEY_SIZE);
    }
    else
    {
        /* Allocate error struct and free the previously allocated prim to eliminate memory leaks. */
    ...
    if (private_key)
    {
        memmove(&prim->local_private_key, private_key, CL_CRYPTO_LOCAL_PVT_KEY_SIZE);
    }
    else
    {
        /* Allocate error struct and free the previously allocated prim to eliminate memory leaks. */
...
void ConnectionEncryptBlockAes(Task theAppTask, uint16 data_array[CL_CRYPTO_AES_DATA_SIZE], uint16 key_array[CL_CRYPTO_AES_KEY_SIZE])
{
    ...
    if (data_array)
    {
        memmove(&prim->data, data_array, CL_CRYPTO_AES_DATA_SIZE);
    }
    else
    {
        /* Allocate error struct and free the previously allocated prim to eliminate memory leaks. */
    ...
    if (key_array)
    {
        memmove(&prim->encryption_key, key_array, CL_CRYPTO_AES_KEY_SIZE);
    }
    else
    {
        /* Allocate error struct and free the previously allocated prim to eliminate memory leaks. */
...
void ConnectionEncryptBlockSha256(Task theAppTask, uint16 *data_array, uint16 array_size)
{
    ...
    prim->phandle   = 0;
    prim->context   = (context_t) theAppTask;
    memset(&prim->data, 0, CL_CRYPTO_SHA_DATA_SIZE);
    ...
    if (array_size <= SHA_DATA_OCTET_SIZE)
    {
    ...
    else
    {
        /* Since the data is larger than a single block of 32 bytes, the first 32 bytes are sent for hashing.
           ...
           hashing is complete. */
        prim->data_size = SHA_DATA_OCTET_SIZE;
        memmove(&prim->data, data_array, CL_CRYPTO_SHA_DATA_SIZE);
...
void ConnectionEncryptBlockSha256Continue(Task theAppTask, uint16 *data_array, uint16 array_size, uint16 current_index)
{
    ...
    prim->phandle   = 0;
    prim->context   = 0;
    memset(&prim->data, 0, CL_CRYPTO_SHA_DATA_SIZE * sizeof(uint16));
    ...
    else
    {
        ...
        prim->data_size = SHA_DATA_OCTET_SIZE;
        memmove(&prim->data, &data_array[current_index / 2], CL_CRYPTO_SHA_DATA_SIZE);
    }
