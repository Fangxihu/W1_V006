...
#define PEER_SYNC_MSG_PAIRING_OFFSET                12
#define PEER_SYNC_MSG_TX_SEQNUM_OFFSET              13
#define PEER_SYNC_MSG_RX_SEQNUM_OFFSET              14
/*!@} */

/*! Defines for boolean elements of the state and pairing components in the peer
 * sync message. */
/*!@{ */
...
#define PEER_SYNC_MSG_PAIRING_HANDSET_COMPLETE      (1 << 0)
#define PEER_SYNC_MSG_PAIRING_HANDSET_IN_PROGRESS   (1 << 1)
#define PEER_SYNC_MSG_STATE2_SCO_ACTIVE             (1 << 4)
#define PEER_SYNC_MSG_STATE2_DFU_IN_PROGRESS        (1 << 5)
#ifdef	RECONNECT_HANDSET
#define PEER_SYNC_MSG_STATE2_HANDSET_RECONNECTED    (1 << 6)
#define PEER_SYNC_MSG_STATE2_HANDSET_RECONNECTING   (1 << 7)
#endif
/*!@} */
...
#define PEER_SYNC_GET_PAIRING(x)        (x[PEER_SYNC_MSG_PAIRING_OFFSET])
...
#define PEER_SYNC_SET_PAIRING(x,val)        (x[PEER_SYNC_MSG_PAIRING_OFFSET] = val)
...
#define PEER_SYNC_PAIRING_HAS_HANDSET_PAIRING(x) \
    ((PEER_SYNC_GET_PAIRING(x) & PEER_SYNC_MSG_PAIRING_HANDSET_COMPLETE) ? TRUE : FALSE)
#define PEER_SYNC_PAIRING_IS_HANDSET_PAIRING(x) \
    ((PEER_SYNC_GET_PAIRING(x) & PEER_SYNC_MSG_PAIRING_HANDSET_IN_PROGRESS) ? TRUE : FALSE)
#define PEER_SYNC_STATE_IS_SCO_ACTIVE(x) \
    ((PEER_SYNC_GET_PAIRING(x) & PEER_SYNC_MSG_STATE2_SCO_ACTIVE) ? TRUE : FALSE)
#define PEER_SYNC_STATE_IS_DFU_IN_PROGRESS(x) \
    ((PEER_SYNC_GET_PAIRING(x) & PEER_SYNC_MSG_STATE2_DFU_IN_PROGRESS) ? TRUE : FALSE)
#ifdef	RECONNECT_HANDSET
#define PEER_SYNC_STATE_IS_HANDSET_RECONNECTED(x) \
    ((PEER_SYNC_GET_PAIRING(x) & PEER_SYNC_MSG_STATE2_HANDSET_RECONNECTED) ? TRUE : FALSE)
#define PEER_SYNC_STATE_IS_HANDSET_RECONNECTING(x) \
	((PEER_SYNC_GET_PAIRING(x) & PEER_SYNC_MSG_STATE2_HANDSET_RECONNECTING) ? TRUE : FALSE)
#endif
/*!@} */
...
#define PEER_SYNC_PAIRING_SET_IS_HANDSET_PAIRING(x) \
    ((x) ? PEER_SYNC_MSG_PAIRING_HANDSET_IN_PROGRESS : 0)
#define PEER_SYNC_PAIRING_SET_HAS_HANDSET_PAIRING(x) \
    ((x) ? PEER_SYNC_MSG_PAIRING_HANDSET_COMPLETE : 0)
...
#ifdef	RECONNECT_HANDSET
#define PEER_SYNC_STATE_SET_HANDSET_RECONNECTED(x) \
    ((x) ? PEER_SYNC_MSG_STATE2_HANDSET_RECONNECTED : 0)
#define PEER_SYNC_STATE_SET_HANDSET_RECONNECTING(x) \
	((x) ? PEER_SYNC_MSG_STATE2_HANDSET_RECONNECTING : 0)
#endif
/*!@} */
...
static void appPeerSyncMsgDebug(const uint8* msg)
{
    ...
    bitfield =  (!!PEER_SYNC_STATE_IS_IN_CASE(msg)            << 12) |
                (!!PEER_SYNC_STATE_IS_IN_EAR(msg)             <<  8) |
                (!!PEER_SYNC_PAIRING_IS_HANDSET_PAIRING(msg)  <<  4) |
                (!!PEER_SYNC_PAIRING_HAS_HANDSET_PAIRING(msg) <<  0);
    DEBUG_LOGF("    appPeerSync (in_case|in_ear|is_pairing|have_handset_pairing)=%04x", bitfield);
}
...
void appPeerSyncSend(bool response)
{
    ...
    if (appDeviceGetPeerBdAddr(&peer_addr))
    {
        ...
        const uint8 pairing = (PEER_SYNC_PAIRING_SET_IS_HANDSET_PAIRING(appGetState() == APP_STATE_HANDSET_PAIRING)) +
                              (PEER_SYNC_PAIRING_SET_HAS_HANDSET_PAIRING(!BdaddrIsZero(&paired_handset_addr))) +
                              (PEER_SYNC_STATE_SET_SCO_ACTIVE(appHfpIsScoActive())) +
#ifdef	  RECONNECT_HANDSET
								(PEER_SYNC_STATE_SET_HANDSET_RECONNECTED(ReconnectGetState() == RECONNECT_STATE_END)) +
								(PEER_SYNC_STATE_SET_HANDSET_RECONNECTING(ReconnectGetState() == RECONNECT_STATE_ING)) +
#endif
                              (PEER_SYNC_STATE_SET_DFU_IN_PROGRESS(appGetState() == APP_STATE_IN_CASE_DFU));
...
bool appPeerSyncIsPeerHandsetReconnected(void)
{
    peerSyncTaskData* ps = appGetPeerSync();
    DEBUG_LOGF("peer_handset_reconnected = %d", ps->peer_handset_reconnected);
    //return appPeerSyncIsComplete() && (ps->peer_handset_tws == DEVICE_TWS_VERSION);
	return ps->peer_handset_reconnected;
}

bool appPeerSyncIsPeerHandsetReconnecting(void)
{
    peerSyncTaskData* ps = appGetPeerSync();
    if (appPeerSyncIsComplete())
        return ps->peer_handset_reconnecting;
    else
        return FALSE;
}
