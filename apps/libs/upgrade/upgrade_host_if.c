...
void UpgradeHostIFProcessDataRequest(uint8 *data, uint16 dataSize)
{
    ...
    if (UpgradeCtxGet()->need_data_cfm)
    {
        if(dataSize)
        {
           dataCfm = (UPGRADE_TRANSPORT_DATA_CFM_T *) PanicUnlessMalloc(sizeof(UPGRADE_TRANSPORT_DATA_CFM_T) + dataSize - 1);
           memset(dataCfm, 0, sizeof(UPGRADE_TRANSPORT_DATA_CFM_T) + dataSize - 1);
        }
        else
        {
        ...
        ByteUtilsMemCpy(dataCfm->data,0, data,0, dataSize);
