...
class HeapPmMem(NewHeapPmMem):
    ...
    def _get_heap_property(self, heap_number):
        ...
        try:
            ...
            patch_available_p1_dis = (patch_level >= 5265)
        ...
        active_num_cores = self.chipdata.get_var_strict('L_active_num_cores').value
        ...
        if patch_available_p1_ext and is_aurachip:
            if heap_number > 2:
                ...
                heap_start = self.debuginfo.get_constant_strict( \
                    '$PM_RAM_END_ADDRESS').value - heap_size
        ...
        try:
            ...
            if heap_number == 0:
                heap_size = self.debuginfo.get_constant_strict('$PM_HEAP_SIZE').value - patch_size
                if patch_available_p1_dis and (active_num_cores == 1) and is_aurachip:
                    # Claim all of P1's PM Heap (including the extra 6K) and cache (2K)
                    # if P1 is disabled
                    p1_pm_cache = 0x800
                    total_p1_pm_heap = self.debuginfo.get_constant_strict('$PM_P1_HEAP_SIZE').value
                    heap_size = heap_size + total_p1_pm_heap + p1_pm_cache
            else:
                if patch_available_p1_dis and (active_num_cores == 1):
                    # Only one heap is supported for this configuration.
                    return (False, 0, 0, 0, 0)
                heap_size = self.debuginfo.get_constant_strict('$PM_P1_HEAP_SIZE').value
                if patch_available_p1_ext and is_aurachip:
                    heap_start = self.debuginfo.get_constant_strict(
