...
#define RULE_EVENT_CONNECTABLE_CHANGE            (1ULL << 47)    /*!< The earbud connectable/not-connectable state changed */
...
typedef enum
{
    RULE_CONNECT_NULL,              /*!< User initiated connection */
...
typedef struct
{
	...
	ruleConnectReason reason;					/*!< Application state */
#endif
} connRulesTaskData; 
...
/*! \brief Register a task to receive notifications that no rules are in progress.
    \param task [IN] Task to receive notifications.
*/
extern void appConnRulesNopClientRegister(Task task);

#ifdef RECONNECT_HANDSET

extern void ruleConnectSetReason(ruleConnectReason reason);

extern ruleConnectReason ruleConnectGetReason(void);
