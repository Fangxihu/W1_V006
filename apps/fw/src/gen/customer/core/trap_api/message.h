...
/**
 *  \brief Associate a Task with a Sink to be run at high priority
 *  When asynchronous data flow notifications (MessageMoreData, MessageMoreSpace)
 *  are 
 ...
 * \ingroup trapset_stream
 */
Task MessageStreamTaskFromSinkHiPri(Sink sink, Task task);

/**
 *  \brief Associate a task with a source to be run at high priority
 *  When asynchronous data flow notifications (MessageMoreData, MessageMoreSpace)
 *  are 
 *  received for any Source or Sink associated with this Task, the handler function
 *  is run immediately in a special mode that "pre-empts" any non-fast blocking 
 ...
 * \ingroup trapset_stream
 */
Task MessageStreamTaskFromSourceHiPri(Source source, Task task);
