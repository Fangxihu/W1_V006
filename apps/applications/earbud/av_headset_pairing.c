...
static void appPairingEnterPeerInquiry(pairingTaskData *thePairing)
{
    ...
    thePairing->inquiry_rssi[0] = thePairing->inquiry_rssi[1] = APP_PAIRING_MIN_RSSI;
...
static void appPairingHandsetComplete(pairingTaskData *thePairing, pairingStatus status, const bdaddr *bd_addr)
{
    ...
    if (thePairing->client_task)
    {
		DEBUG_LOG("thePairing->client_task!");
...
/*! \brief Handle authentication confirmation
    ...
    TWS+.
*/
void appPairingHandleClSmAuthenticateConfirm(const CL_SM_AUTHENTICATE_CFM_T *cfm)
{
    pairingTaskData *thePairing = appGetPairing();
...
*/
bool appPairingHandleClSmAuthoriseIndication(const CL_SM_AUTHORISE_IND_T *ind)
{
    pairingTaskData *thePairing = appGetPairing();
    ...
    {
        /* Check if connection is to peer, if it is, it's nothing to do with handset pairing, so let
           connection mananger deal with authorisation */
...
*/
void appPairingHandleClSmIoCapabilityReqIndication(const CL_SM_IO_CAPABILITY_REQ_IND_T *ind)
{
    pairingTaskData *thePairing = appGetPairing();
    bool accept = FALSE;
    ...
    switch (appPairingGetState(thePairing))
    {
        ...
        case PAIRING_STATE_IDLE:
            /* Check for the Cross Transport Key derivation(CTKD) case and accpet
               if this is from a device that we know about */
            if ((ind->sm_over_bredr) &&
                (ind->tpaddr.transport == TRANSPORT_BLE_ACL) &&
                (appDeviceFindBdAddrAttributes(&ind->tpaddr.taddr.addr, NULL)))
            {
                accept = TRUE;

                /* We use a different key distribution for LE */
...
void appPairingHandleClSmRemoteIoCapabilityIndication(const CL_SM_REMOTE_IO_CAPABILITY_IND_T *ind)
{
    UNUSED(ind);
        ...
        case PAIRING_STATE_PEER_INQUIRY:
        {
            ...
            else
            {
                ...
                if (!BdaddrIsZero(&thePairing->bd_addr[0]))
                {
                    ...
                    if ((thePairing->inquiry_rssi[1] - thePairing->inquiry_rssi[0]) <= APP_PAIRING_MIN_RSSI_DELTA)
                    {
...
void appPairingHandleClSmSecurityModeConfigConfirm(const CL_SM_SEC_MODE_CONFIG_CFM_T *cfm)
{
...
*/
void appPairingHandleClPinCodeIndication(const CL_SM_PIN_CODE_IND_T *ind)
{
    uint16 pin_length = 0;
    uint8 pin[16];
...
/*  Handle the user passkey confirmation.
    ...
    immediately.
*/
void appPairingHandleClSmUserConfirmationReqIndication(const CL_SM_USER_CONFIRMATION_REQ_IND_T *ind)
{
...
*/
void appPairingHandleClSmUserPasskeyReqIndication(const CL_SM_USER_PASSKEY_REQ_IND_T *ind)
{
...
void appPairingHandsetPair(Task client_task, bool is_user_initiated)
{
    ...
    MessageSendConditionally(&thePairing->task, PAIRING_INTERNAL_HANDSET_PAIR_REQ,
                             message, &thePairing->pairing_lock);
    DEBUG_LOG("appPairingHandsetPair~");
}
...
void appPairingHandsetPairAddress(Task client_task, bdaddr* handset_addr)
{
    ...
    MessageSendConditionally(&thePairing->task, PAIRING_INTERNAL_HANDSET_PAIR_REQ,
                             message, &thePairing->pairing_lock);
    DEBUG_LOG("appPairingHandsetPairAddress~");
}
