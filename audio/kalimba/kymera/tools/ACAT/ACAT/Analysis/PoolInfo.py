...
class PoolInfo(Analysis.Analysis):
    ...
    def display_configuration(self):
        ...
        if self.pmalloc_debug_enabled:
            output_str += \
                "* for each block, three words are used for internal tracking data."
        else:
            output_str += \
    ...
    def _inspect_pool(self, pool):
        ...
        while free_ptr != 0:
            if free_ptr in all_block_pointers:
                ...
                pool_errors.append(
                    "Free-list pointer out of range, Pool: {0:>2d} Ptr: 0x{1:0>8x}".
            ...
            if self.pmalloc_debug_enabled:
                ...
                if not blockvar.free:
                    ...
                    if line == 0:
                        ...
                        block_debug_info = (
                            "Ptr: 0x{0:0>8x} size: {1:>3d} allocated from pool: "
                            "{2:>2d} by: {3:s}".format(
        ...
        for i, blockvar in enumerate(poolvar.members):
            if (not blockvar.owned) and (not blockvar.free):
                pool_errors.append(
                    "Unowned block, Pool: {0:>2d} Block {1:>3d} Ptr: 0x{2:0>8x}".
