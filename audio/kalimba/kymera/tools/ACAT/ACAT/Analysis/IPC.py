...
class IPC(Analysis.Analysis):
    """
    @brief This class encapsulates an analysis for Kymera inter-processor
        communication.
    """
    ...
    def print_message_queue(
            self, msg_queue_var, display_buffer, display_messages
        ):
        '''
        ...
        self.formatter.output(
            ...
            format(cu.hex(buffer_var.get_member('base_addr').value))
        )
    ...
    def analyse_messages(self, display_buffer=True, display_messages=False):
        ...
        for msg_channel_p in msg_cannel_p_array:
            ...
            self.formatter.output(
                '  Channel ID: {0}\n'.
                format(cu.hex(msg_channel.get_member('channel_id').value))
            )
    ...
    def _display_signal_block(self, signal_p, analyse_fifo):
        ...
        channels_p = signal_block.get_member('channels').value
        ...
        channel_type_size = self.debuginfo.get_type_info('IPC_SIGNAL_CHANNEL')[5]
        for channel_nr in range(active_chans):
            ...
            if channel.get_member('signal_owner').value == free_channel_bit:
                ...
                chanels_str += ' ' * 8 + 'Signal channel ID: {0}\n'.format(
                    cu.hex(channel.get_member('signal_id').value)
                )
                chanels_str += ' ' * 8 + 'Priority: {0}\n'.format(
                    cu.hex(channel.get_member('priority').value)
                )
                chanels_str += ' ' * 8 + 'Signal Owner: {0}\n'.format(
                    cu.hex(channel.get_member('signal_owner').value)
                )
                try:
                    module_name = self.debuginfo.get_source_info(
                ...
                if signal_data.get_member('signal_size').value == 0:
                    ...
                    chanels_str += 'Signal of size {0} at {1}\n\n'.format(
                        cu.hex(signal_data.get_member('signal_size').value),
                        cu.hex(signal_data.get_member('signal_ptr').value)
                    )
            ...
            for data_cannel in data_cannels:
                self.formatter.output(
                    ' ' * 6 + 'Channel ID: {0}'.format(
                        cu.hex(data_cannel.get_member('data_channel_id').value)
                    )
                )
    ...
    def analyse_aux_states(self):
        ...
        for processor in range(aux_states_array.array_len):
            ...
            self.formatter.output(
                '    Setup state: {0} - {1}'.
                format(cu.hex(setup_state), state_str)
            )
            ...
            if fifo.get_member('max').value == 0:
                ...
                if numelts > 0:
                    ...
                    for element_nr in range(numelts):
                        ...
                        element = fifo.get_member('entry').__getitem__(read_idx)
                        self.formatter.output(
                            ' ' * 4 +
    ...
    def _get_buffer_messages(self, buffer_var):
        ...
        # about the message
        while True:
            # now read the messages that were in this buffer before they were read.
            # They will be ones before read pointer do it until current pointer
            # reaches write pointer, when reading backwards. Do not forget about
            # buffer wrapping
    ...
    def _lookup_debuginfo(self):
        '''
        @brief this method reads the information required for analyses.
