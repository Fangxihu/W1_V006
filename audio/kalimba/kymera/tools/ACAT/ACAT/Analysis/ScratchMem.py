...
class ScratchMem(Analysis.Analysis):
    """
    @brief This class encapsulates an analysis for usage of scratch memory.
    """
    ...
    def run_all(self):
        """
        @brief Perform all useful analysis and direct the output via the Analysis'formatter.
        It analyses the scratch memory usage in DM1, DM2 and none, per priority and per
        task.
    ...
    def analyse_per_task(self, task_id=None):
        """
        @brief analyse_per_task() displays the scratch memory info per task.
        It can take one parameter that represents the task id and it will display the info
        for it.
        The function goes through the linked list and if no value is passed to it,
        it displays the complete list. Otherwise, it displays the info for the task with the
        id inputted or a message (if the task id is invalid).
        @param[in] self Pointer to the current object
