...
{
    if (
            (flags == ble_read_random_address_peer) &&
            (peer_tpaddr == NULL)
       )
    {
        CL_DEBUG(("Cannot read random address of peer if its address is NULL\n"));
    }
    else if (
            flags < ble_read_random_address_local || 
            flags > ble_read_random_address_peer
            )
    {
    ...
    else
    {
        ...
        if (flags == ble_read_random_address_local)
        {
            prim->flags = DM_SM_READ_ADDRESS_LOCAL;
            memset(&prim->tp_peer_addrt, 0, sizeof(TP_BD_ADDR_T));
        }
        /* Otherwise, flags must be ble_read_random_address_peer */
        else
        {
