...
#if THIS_PROCESSOR == 0
    // If running from TCM then lock the cache before trying to copy
    // the .initc into RAM (the values might be in the locked cache)
    // Only processor 0 needs thei: Processor 1 will have its TCM and
    // cache set up by processor 0 before it is set running.
    r0 = 0;
    r1 = M[$APPS_SYS_CPU0_TCM_REMAP_CONTROLS];
    Null = r1 AND $TCM_REMAP_LOW_ENABLE_MASK;
    if NZ call $_lock_cache;
#endif
...
#if THIS_PROCESSOR == 0
    // Implementation of putchar for the digital simulation and kalsim
$_putchar:
    kalcode(0x8d018000);
    rts;
#endif
