...
static void process_data(gaia_transport *transport, uint16 size_data, uint8 *data)
{
   ...
   {
       gaiaProcessData(transport, size_data, data);
   }
   else
   {
       UNUSED(transport);
...
bool gaiaTransportGattProcessSource(void)
{
    ...
    if(packetSize)
    {
        ...
        process_data(gaia->upgrade_transport, ((packetSize)-GAIA_HANDLE_SIZE), (p_source + GAIA_HANDLE_SIZE));

        if(RwcpGetOutOfSequenceStatus())
        {
