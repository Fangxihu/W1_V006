...
static const operator_path_t paths_mono_or_stereo[] = 
{
...
static const chain_config_t speaker_config_mono_or_stereo =
    MAKE_CHAIN_CONFIG_WITH_PATHS(chain_id_audio_mixer_speaker,
                                   audio_ucid_mixer_speaker_stereo,
                                   speaker_ops_mono_or_stereo,
                                   paths_mono_or_stereo);
    ...
    switch(chain_type)
    {
        case audio_mixer_speaker_mono:
            if(mono_path_enabled)
                return &speaker_config_mono_or_stereo;
            else
