...
class Analysis(object):
    ...
    data structures rather than printing it directly.
    An analysis which prints information via the formatter should follow the naming
    convention that has the form analyse_xxx (or run_all). These functions return nothing.
    ...
    An analysis can cache some of its answer. In other words the return value for some
    of their functions can be cached. Function which are used for inter-analyses
    communication should always be cached. In this manner, we can avoid refreshing the internal
    data of the called analysis and creating new variables when providing information
    to other analyses. To mark a function for caching it must be decorated
    with cache_decorator.  This cache is cleared after each user instruction in
    interactive mode. With coredumps or in automatic mode there is no need to empty the cahce.
    """
    ...
    def __init__(self, **kwargs):
        ...
        try:
            if self.chipdata is None or self.debuginfo is None \
                or self.formatter is None:
                raise Exception(
            ...
            raise Exception(
                "All analyses need at least chipdata, debuginfo and formattter."
            )
    ...
    def parse_linked_list(
            self, identifier, next_field_name, override_element_type=None, elf_id=None
        ):
        """
        @brief A helper function for traversing a linked-list structure. This is a
        generator and yields each element of the list. The input parameters are:
        * identifier - the variable at the head of the list. This can be the
        name or address of a (named) variable in firmware, or a Variable object
        (e.g. the result of a call to Analysis.cast).
        Note that this could be the first element in the list, or a pointer to the
        head of the list (e.g. '$_transform_list').
        * next_field_name - the name of the field in the structure that points to the
        next element in the list.
        * override_element_type - by default each list member is interpreted as the
        type as which the next_field_name pointer is declared. Optionally specify
        another type to cast to.
        @param[in] self Pointer to the current object
        @param[in] identifier
        ...
        while element_ptr.value != 0:
            # Check if the list terminated with 0xffffff(-1 in 24 bit = 16777215
            # in 24bit) or 0xFFFFFFFF (32 bit). If so, raise and error because
            ...
            if override_element_type is not None:
                ...
                element = self.chipdata.cast(
                    ...
                    deref_type=True,
                    elf_id=element_ptr.debuginfo.elf_id if element_ptr.debuginfo else None,
                )
            yield element
    ...
    def find_references(self, value):
        """
        @brief Searches the whole of DM for the supplied value. Returns a tuple
        of each address that contains that value (empty tuple if it
        was not found anywhere).
        Note: it does not search in memory-mapped registers, only in DM1 and DM2.
        @param[in] self Pointer to the current object
        @param[in] value
        """
        if Arch.kal_arch != 4:
