...
static void populateTuningConfigData(anc_config_t * config_data)
{
    uint16 read_buffer[ANC_TUNING_CONFIG_DATA_SIZE];

    if(readTuningKey(ANC_ACTIVE_MODE_TUNING_CONFIG_KEY, read_buffer))
    {
        populateInstance(&config_data->active.instance[ANC_INSTANCE_0_INDEX], &read_buffer[INSTANCE_0_OFFSET]);
        populateInstance(&config_data->active.instance[ANC_INSTANCE_1_INDEX], &read_buffer[INSTANCE_1_OFFSET]);

        /* Hardware gains are always set by the active key, any gains set in the leakthrough key are ignored */
    ...
    if(readTuningKey(ANC_LEAKTHROUGH_MODE_TUNING_CONFIG_KEY, read_buffer))
    {
        populateInstance(&config_data->leakthrough.instance[ANC_INSTANCE_0_INDEX], &read_buffer[INSTANCE_0_OFFSET]);
        populateInstance(&config_data->leakthrough.instance[ANC_INSTANCE_1_INDEX], &read_buffer[INSTANCE_1_OFFSET]);
    }
    else
    {
        config_data->leakthrough = config_data->active;
    }
}
...
static void populateDeviceSpecificAncPathGains(anc_config_t * config_data)
{
    if(isPsKeyValid(ANC_PATH_TUNING_KEY, production_anc_path_gain_index_max))
    {
        ...
        populateAncPathGain(&config_data->active.instance[ANC_INSTANCE_0_INDEX].feed_forward_a.gain_config,
                path_tuning_data[production_anc_path_gain_index_active_mode_instance_0_feed_forward_a]);
        populateAncPathGain(&config_data->active.instance[ANC_INSTANCE_0_INDEX].feed_forward_b.gain_config,
                path_tuning_data[production_anc_path_gain_index_active_mode_instance_0_feed_forward_b]);
        populateAncPathGain(&config_data->active.instance[ANC_INSTANCE_0_INDEX].feed_back.gain_config,
                path_tuning_data[production_anc_path_gain_index_active_mode_instance_0_feed_back]);

        populateAncPathGain(&config_data->active.instance[ANC_INSTANCE_1_INDEX].feed_forward_a.gain_config,
                path_tuning_data[production_anc_path_gain_index_active_mode_instance_1_feed_forward_a]);
        populateAncPathGain(&config_data->active.instance[ANC_INSTANCE_1_INDEX].feed_forward_b.gain_config,
                path_tuning_data[production_anc_path_gain_index_active_mode_instance_1_feed_forward_b]);
        populateAncPathGain(&config_data->active.instance[ANC_INSTANCE_1_INDEX].feed_back.gain_config,
                path_tuning_data[production_anc_path_gain_index_active_mode_instance_1_feed_back]);

        populateAncPathGain(&config_data->leakthrough.instance[ANC_INSTANCE_0_INDEX].feed_forward_a.gain_config,
                path_tuning_data[production_anc_path_gain_index_leakthrough_mode_instance_0_feed_forward_a]);
        populateAncPathGain(&config_data->leakthrough.instance[ANC_INSTANCE_0_INDEX].feed_forward_b.gain_config,
                path_tuning_data[production_anc_path_gain_index_leakthrough_mode_instance_0_feed_forward_b]);
        populateAncPathGain(&config_data->leakthrough.instance[ANC_INSTANCE_0_INDEX].feed_back.gain_config,
                path_tuning_data[production_anc_path_gain_index_leakthrough_mode_instance_0_feed_back]);

        populateAncPathGain(&config_data->leakthrough.instance[ANC_INSTANCE_1_INDEX].feed_forward_a.gain_config,
                path_tuning_data[production_anc_path_gain_index_leakthrough_mode_instance_1_feed_forward_a]);
        populateAncPathGain(&config_data->leakthrough.instance[ANC_INSTANCE_1_INDEX].feed_forward_b.gain_config,
                path_tuning_data[production_anc_path_gain_index_leakthrough_mode_instance_1_feed_forward_b]);
        populateAncPathGain(&config_data->leakthrough.instance[ANC_INSTANCE_1_INDEX].feed_back.gain_config,
                path_tuning_data[production_anc_path_gain_index_leakthrough_mode_instance_1_feed_back]);
    }
}
...
{
    populateTuningConfigData(config_data);
