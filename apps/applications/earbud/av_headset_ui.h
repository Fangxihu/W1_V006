...
#define appUiReconnectingInactive(is_user_initiated) \
    appLedStopPattern(LED_PRI_MED)
#endif
...
extern void appUiPlayPromptCore(voicePromptName prompt, bool interruptible,
                                uint16 *client_lock, uint16 client_lock_mask);
...
/*! \brief Play a tone allowing another tone/prompt/event to interrupt (stop) this tone
     before completion. */
#define appUiPlayToneInterruptible(tone) appUiPlayToneCore(tone, TRUE, NULL, 0)
/*! \brief Play a tone to completion. mask bits will be cleared in lock
    when the tone completes, or if it is not played at all. */
#define appUiPlayToneClearLock(tone, lock, mask) appUiPlayToneCore(tone, FALSE, lock, mask)
/*! \brief Play a tone allowing another tone/prompt/event to interrupt (stop) this tone
...
#define appUiPlayToneInterruptibleClearLock(tone, lock, mask) appUiPlayToneCore(tone, TRUE, lock, mask)

/*! \brief Play a prompt to completion */
#define appUiPlayPrompt(prompt) appUiPlayPromptCore(prompt, FALSE, NULL, 0)
/*! \brief Play a prompt allowing another tone/prompt/event to interrupt (stop) this prompt
     before completion. */
#define appUiPlayPromptInterruptible(prompt) appUiPlayPromptCore(prompt, TRUE, NULL, 0)
...
#define appUiPlayPromptClearLock(prompt, lock, mask) appUiPlayPromptCore(prompt, FALSE, lock, mask)
/*! \brief Play a prompt allowing another tone/prompt/event to interrupt (stop) this prompt
     before completion. mask bits will be cleared in lock when the prompt completes or
     is interrupted, or if it is not played at all. */
#define appUiPlayPromptInterruptibleClearLock(prompt, lock, mask) appUiPlayPromptCore(prompt, TRUE, lock, mask)
